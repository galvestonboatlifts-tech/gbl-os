...
export default function App() {
  ...
  const job = (recordId) => records.jobs.find((x) => x.id === recordId)
  ...
  const estimatedProfit = records.jobs.reduce(
    (sum, x) => sum + Number(x.contract_amount) - Number(x.material_cost) - Number(x.labor_cost) - Number(x.other_cost),
    0
  )
    ...
    return (
      <article className="card" key={x.id}>
        <div className="between">
          <div>
            <h3>{x.number} · {x.title}</h3>
            <div className="small">{customer(x.customer_id)?.name} · {property(x.property_id)?.address}</div>
          </div>
          <Badge>{x.status}</Badge>
        </div>
        <p>Start {dateText(x.start_date)} · Due {dateText(x.due_date)} · Crew: {x.crew || 'Unassigned'}</p>
        <div className="small">
          Contract {money(x.contract_amount)} · Estimated profit{' '}
          <span className={profit >= 0 ? 'positive' : 'negative'}>{money(profit)}</span>
        </div>
        <div className="actions">
          <button className="secondary" onClick={() => setEditor({ type: 'job', row: x })}>Open Job</button>
          <button className="secondary" onClick={() => setEditor({ type: 'photos', row: x })}>Photos</button>
          <button className="danger" onClick={() => remove('jobs', x.id)}>Delete</button>
        </div>
      </article>
    )
  }
    ...
    return (
      <>
        <div className="metrics">
          <Metric label="Customers" value={records.customers.length} />
          <Metric label="Open Estimates" value={openEstimates.length} />
          <Metric label="Active Jobs" value={activeJobs.length} />
          <Metric label="Receivables" value={money(receivables)} />
          <Metric label="Inventory Items" value={records.inventory.length} />
          <Metric label="Estimated Job Profit" value={money(estimatedProfit)} />
          <Metric label="Maintenance Due" value={records.maintenance.filter((x) => x.status !== 'Completed').length} />
          <Metric label="Cloud Status" value="Connected" />
        </div>
        <div className="grid-2">
          <section className="panel">
            <div className="between"><h2>Active Jobs</h2><button className="ghost" onClick={() => setPage('jobs')}>View all</button></div>
            <div className="cards">{activeJobs.slice(0, 6).map(jobCard)}{!activeJobs.length && <Empty>No active jobs</Empty>}</div>
          </section>
          <section className="panel">
            <h2>Estimate Pipeline</h2>
            <div className="cards">
              {openEstimates.slice(0, 6).map((x) => (
                <article className="card" key={x.id}>
                  <div className="between">
                    <div><h3>{x.number} · {x.title}</h3><div className="small">{customer(x.customer_id)?.name}</div></div>
                    <div><div className="money">{money(x.total)}</div><Badge>{x.status}</Badge></div>
                  </div>
                </article>
              ))}
              {!openEstimates.length && <Empty>No open estimates</Empty>}
            </div>
          </section>
        </div>
      </>
  ...
  function propertiesPage() {
    return <><Toolbar add="Property" onAdd={() => setEditor({ type: 'property', row: {} })} />
      <div className="cards">{records.properties.map((x) => (
        <article className="card" key={x.id}>
          <div className="between">
            <div><h3>{x.address}</h3><div className="small">{customer(x.customer_id)?.name}</div></div>
            <div className="actions"><button className="secondary" onClick={() => setEditor({ type: 'property', row: x })}>Edit</button><button className="danger" onClick={() => remove('properties', x.id)}>Delete</button></div>
          </div>
          <p>{x.lift_details}</p><div className="small">{x.dock_details} · {x.electrical} · {x.water_depth}</div>
        </article>
      ))}{!records.properties.length && <Empty>No properties</Empty>}</div></>
  }

  function estimatesPage() {
    return <><Toolbar add="Estimate" onAdd={() => setEditor({ type: 'estimate', row: {} })} />
      <div className="cards">{records.estimates.map((x) => (
        <article className="card" key={x.id}>
          <div className="between">
            <div><h3>{x.number} · {x.title}</h3><div className="small">{customer(x.customer_id)?.name} · {property(x.property_id)?.address}</div></div>
            <div><div className="money">{money(x.total)}</div><Badge>{x.status}</Badge></div>
          </div>
          <p>{x.scope}</p>
          <div className="actions"><button className="secondary" onClick={() => setEditor({ type: 'estimate', row: x })}>Edit</button><button className="primary" onClick={() => convertEstimate(x)}>Create Job</button><button className="danger" onClick={() => remove('estimates', x.id)}>Delete</button></div>
        </article>
      ))}{!records.estimates.length && <Empty>No estimates</Empty>}</div></>
  }

  function jobsPage() {
    return <><Toolbar add="Job" onAdd={() => setEditor({ type: 'job', row: {} })} /><div className="cards">{records.jobs.map(jobCard)}{!records.jobs.length && <Empty>No jobs</Empty>}</div></>
  }

  function invoicesPage() {
    return <><Toolbar add="Invoice" onAdd={() => setEditor({ type: 'invoice', row: {} })} />
      <div className="table-wrap"><table><thead><tr><th>Invoice</th><th>Customer</th><th>Total</th><th>Paid</th><th>Balance</th><th>Status</th><th></th></tr></thead>
      <tbody>{records.invoices.map((x) => <tr key={x.id}><td><b>{x.number}</b></td><td>{customer(x.customer_id)?.name}</td><td>{money(x.total)}</td><td>{money(x.paid)}</td><td>{money(Number(x.total)-Number(x.paid))}</td><td><Badge>{x.status}</Badge></td><td><button className="secondary" onClick={() => setEditor({ type: 'invoice', row: x })}>Edit</button></td></tr>)}</tbody></table></div></>
  }

  function inventoryPage() {
    return <><Toolbar add="Inventory Item" onAdd={() => setEditor({ type: 'inventory', row: {} })} />
      <div className="table-wrap"><table><thead><tr><th>Part</th><th>Description</th><th>Category</th><th>Cost</th><th>Sell</th><th>Stock</th><th>Location</th><th></th></tr></thead>
      <tbody>{records.inventory.map((x) => <tr key={x.id}><td>{x.part_number}</td><td><b>{x.description}</b></td><td>{x.category}</td><td>{money(x.unit_cost)}</td><td>{money(x.sell_price)}</td><td>{Number(x.quantity)<=Number(x.reorder_level)?<Badge>{x.quantity} Reorder</Badge>:x.quantity}</td><td>{x.location}</td><td><button className="secondary" onClick={() => setEditor({ type: 'inventory', row: x })}>Edit</button></td></tr>)}</tbody></table></div></>
  }

  function employeesPage() {
    return <><Toolbar add="Employee" onAdd={() => setEditor({ type: 'employee', row: {} })} />
  ...
  function maintenancePage() {
    return <><Toolbar add="Reminder" onAdd={() => setEditor({ type: 'maintenance', row: {} })} />
  ...
  function reportsPage() {
    const contracts = records.jobs.reduce((s,x)=>s+Number(x.contract_amount),0)
    const costs = records.jobs.reduce((s,x)=>s+Number(x.material_cost)+Number(x.labor_cost)+Number(x.other_cost),0)
    const invoiced = records.invoices.reduce((s,x)=>s+Number(x.total),0)
    const paid = records.invoices.reduce((s,x)=>s+Number(x.paid),0)
    return <div className="metrics"><Metric label="Contracts" value={money(contracts)}/><Metric label="Estimated Costs" value={money(costs)}/><Metric label="Estimated Gross Profit" value={money(contracts-costs)}/><Metric label="Invoiced" value={money(invoiced)}/><Metric label="Collected" value={money(paid)}/><Metric label="Receivables" value={money(invoiced-paid)}/></div>
  }
  ...
  return (
    <div className="shell">
      <aside className="sidebar">
        <div className="brand"><div className="logo">GBL</div><div><strong>Galveston Boat Lifts</strong><span>Cloud Business OS</span></div></div>
        <nav>{modules.map(([key,label])=><button key={key} className={page===key?'active':''} onClick={()=>setPage(key)}>{label}</button>)}</nav>
        <div className="sidebar-foot">{session.user.email}<button onClick={()=>supabase.auth.signOut()}>Sign out</button></div>
      </aside>
      <main className="main">
        <header className="header"><div><h1>{modules.find(([key])=>key===page)?.[1]}</h1><p>{new Date().toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric',year:'numeric'})}</p></div></header>
        <section className="content">{views[page]()}</section>
      </main>
      {editor && <EditorModal editor={editor} close={()=>setEditor(null)} save={save} records={records} session={session} />}
    </div>
  )
}

function Splash({ text }) {
  return <div className="center"><div className="auth-card"><h1>{text}</h1></div></div>
}

function ConfigurationScreen() {
  return <div className="center"><div className="auth-card"><h1>Configuration required</h1><p>Add these Netlify environment variables, then redeploy:</p><pre>VITE_SUPABASE_URL{'\n'}VITE_SUPABASE_PUBLISHABLE_KEY</pre></div></div>
}
...
function AuthScreen() {
  ...
  async function signIn(e){e.preventDefault();const {error}=await supabase.auth.signInWithPassword({email,password});setMessage(error?.message||'Signing in…')}
  async function signUp(){const {error}=await supabase.auth.signUp({email,password});setMessage(error?.message||'Account created. Check your email if confirmation is enabled.')}
  return <div className="center"><form className="auth-card" onSubmit={signIn}><h1>Galveston Boat Lifts</h1><p>Secure cloud business login</p><Field label="Email" value={email} onChange={setEmail} type="email" required/><Field label="Password" value={password} onChange={setPassword} type="password" required/><button className="primary full">Sign In</button><button type="button" className="secondary full" onClick={signUp}>Create First Account</button><div className="small">{message}</div></form></div>
}

function Toolbar({ add, onAdd }) {
  return <div className="toolbar"><div/><button className="primary" onClick={onAdd}>+ {add}</button></div>
}

function EditorModal({ editor, close, save, records, session }) {
  if (editor.type === 'photos') return <PhotosModal job={editor.row} close={close} session={session}/>
  return <RecordEditor type={editor.type} initial={editor.row} close={close} save={save} records={records}/>
}
...
function RecordEditor({ type, initial, close, save, records }) {
  ...
  const propertyOptions=records.properties.map((p)=>[p.id,p.address])
  ...
  return <Modal title={`${initial.id?'Edit':'New'} ${type}`} onClose={close}>
    <form onSubmit={submit}>
      ...
      {['estimate','invoice'].includes(type)&&<section className="panel line-panel"><div className="between"><h2>Line Items</h2><button type="button" className="secondary" onClick={()=>setLines((old)=>[...old,{description:'',qty:1,price:0}])}>+ Line</button></div><div className="cards">{lines.map((line,index)=><div className="form-grid line-row" key={index}><Field label="Description" value={line.description} onChange={(v)=>setLines((old)=>old.map((item,i)=>i===index?{...item,description:v}:item))}/><Field label="Qty" value={line.qty} onChange={(v)=>setLines((old)=>old.map((item,i)=>i===index?{...item,qty:Number(v)}:item))} type="number"/><Field label="Unit Price" value={line.price} onChange={(v)=>setLines((old)=>old.map((item,i)=>i===index?{...item,price:Number(v)}:item))} type="number"/></div>)}</div><h3>Total: {money(total)}</h3></section>}
      <div className="actions"><button className="primary">Save</button></div>
    </form>
  </Modal>
}
...
function PhotosModal({ job, close, session }) {
  ...
  useEffect(()=>{load()},[])
  async function load(){
    const {data,error}=await supabase.from('job_photos').select('*').eq('job_id',job.id).order('created_at',{ascending:false})
    if(error)return alert(error.message)
    const signed=[]
    for(const photo of data||[]){
      const {data:url}=await supabase.storage.from('job-photos').createSignedUrl(photo.storage_path,3600)
      signed.push({...photo,url:url?.signedUrl})
    }
    setPhotos(signed)
  }
